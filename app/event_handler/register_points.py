...
#        session = registry.get(point.asset.site.db_key)
    #    if not session is None:
        ...
        #cv changed this next line to db.session.query from session.query to remove webreports dependancy
        loggedentity = db.session.query(LoggedEntity).filter_by(descr=identifier).first()
